...
.ready(
		function() {	
		    ...
		    function clickHandler(event) {  
		        ...
		        var follow_id=$("#current"+i).attr("data");
		        var url="${pageContext.request.contextPath}/jsp/other_center/otherscenter.jsp?id="+following_id;
		        var status=$("#current___"+i).attr("title");
		        if(status=="未读") status="已读";
		        else if(status="已读") status="未读";
		        var url2="${pageContext.request.contextPath}/servlet/UpdateFollow";
		        var args={
		        		"follow_id" : follow_id,
		        		"status":status,
						"time" : new Date()
...
<!--评论消息begin--> 
         <div id="messagecenter_reviews">
            ...
            <select id="select"  style="width:150px; float:right; margin-right:20px; font-size:18px;">
		 ...
		 <%
							...
							if (!follows.isEmpty()) {
								...
								for (int i = 0; i < follows.size(); i++) {
									...
									int following_id=follow.getFollowing_id();
									User following = service.getUserFromId(following_id);
									
						%>

						<%
							if (service.getFollowRecord(user_id,following_id)&&follow.getStatus().equals("未读")) {
						%>
						
						<tr id="current____<%=j%>">

							<td><%=j + 1%></td>
							<td>你关注的<a href="javascript:;" id="current_<%=j %>" data="<%=following.getId() %>" style="color: #339b53;"><%=following.getUsername()%></a>
								也关注了你<span style="display:none;" id="current<%=j%>" data="<%=follow.getId()%>"></span></td>
						...
						<%
						++j;}else if(follow.getStatus().equals("未读")){
						%>

						<tr id="current____<%=j%>">

							<td><%=j + 1%></td>
							<td><a href="javascript:;" id="current_<%=j %>" data="<%=following.getId() %>" style="color: #339b53;"><%=following.getUsername()%></a>
								关注了你<span style="display:none;" id="current<%=j%>" data="<%=follow.getId()%>"></span></td>
							<td><%=follow.getFollow_date()%></td>
							<td><a id="current__<%=j%>" style="cursor:pointer"><img id="current___<%=j %>"
								src="${pageContext.request.contextPath}/image/unread.png" title="未读"
								style="width: 40px; height: 40px;"></a></td>
    ...
    <div id="hiddenresult2" style="display: none;">
	    ...
	    <th width="15%">操作</th>
		 ...
		 <%
						...
						<%
							if (service.getFollowRecord(user_id,following_id)&&follow.getStatus().equals("已读")) {
						%>
						...
						<%
						++j;}else if(follow.getStatus().equals("已读")){
						%>
